 ...
 * Acts as a controller in the MVC pattern - delegates business logic to AuthService
 */
...
public class ProfileServlet extends HttpServlet {
    ...
    private void handleProfileUpdate(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // Update profile via service (validates input and uniqueness)
            User updatedUser = authService.updateProfile(currentUser.getId(), username, email, firstName, lastName);
            
            // Handle optional password change
            if (currentPassword != null && !currentPassword.trim().isEmpty() &&
                newPassword != null && !newPassword.trim().isEmpty()) {
                try {
                    authService.changePassword(updatedUser.getId(), currentPassword, newPassword);
                    logger.info("Password changed for user: " + updatedUser.getUsername());
